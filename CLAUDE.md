...
| project-manager | https://claude.ai/artifact/Qperj1e8cxMEMgtFKV55M5 |
...
### Pipeline sync
**Whenever `clients/pipeline.md` changes**, sync it straight afterwards to **both** the HQ page and the project-manager page. Use the `ArtifactData` tool (load it with ToolSearch if needed): action `set`, collection `pipeline`, doc id `current`, with this body built from the whole of `clients/pipeline.md`:
...
2. **Updates:** on the project-manager page, `query` the `updates` collection the same way. Each update has `business`, `event` (e.g. "They paid the £595"), `date` and `note`. Pass them to the **project-manager** subagent, oldest first, to update `clients/pipeline.md`, then mark each one applied the same way.
