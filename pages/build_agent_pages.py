...
AGENTS = {
  "idea-creator": {
    ...
    "rules": [
      ...
      ("Always", ["Says exactly £595 to build and £50 a month", "Contacts by email first (Gmail drafts), then text. Avoids Instagram and Facebook DMs so your accounts stay safe", "Signs off \"Sonny\"", "Keeps texts under 60 words and emails under 120", "Uses one specific detail about the business and ends with one easy question", "Only calls you local around Shrewsbury, and offers the Annie example site to beauty businesses only"]),
...
function finderText() {
  ...
  return `Use the idea-creator agent to find ${g("fCount") || 8} ${g("fTrade") || "small businesses"} in ${g("fTown") || "Shrewsbury"} with no website and draft ${g("fChan") === "email, then text" ? "pitches by email where there is a business email, otherwise by text, with no Instagram or Facebook DMs" : g("fChan") + " pitches (no Instagram or Facebook DMs)"}. Then have the critique agent check every one, apply its fixes, save any email pitches as Gmail drafts, sync the leads to the idea-creator page, and show me the final messages.`;
}
