---
description: Build and polish a client site draft (web-builder → critique → web-builder → critique → boss)
...
5. **Track.** Use the **boss** subagent to put the business in **In progress** (move it if it's already in the pipeline) with the site folder, the target delivery from the turnaround in `business.md` (default 5 working days), and "Waiting on: client info" if placeholders remain. Then sync the pipeline to Sonny's pages as described in `CLAUDE.md`.
6. **Report back** with:
