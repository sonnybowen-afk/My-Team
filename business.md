# My business details

Every agent reads this file. **Fill in the `[EDIT]` bits once** and every pitch, site footer and follow-up will use them.

## Me
- **Full name:** Sonny Bowen
- **First name (for signing messages):** Sonny
- **Trading name (optional):** none, I go by my name
- **Where I'm based / areas I cover:** Based in Shrewsbury. Happy to take on businesses in other towns across England too. Only call myself "local" when pitching Shrewsbury and nearby Shropshire towns. Elsewhere, just say I build websites for small businesses.
- **UK mobile (calls and texts):** 07944 539622 (+44 7944 539622)
- **WhatsApp:** Thai number 093 121 6854 (+66 93 121 6854, wa.me/66931216854). Use this only for WhatsApp, never for calls or SMS. UK businesses will see a +66 number, so WhatsApp pitches should say it's my WhatsApp and give the UK mobile for calls.
- **Email:** sonnybowen07@gmail.com
- **Example site to show people:** https://sonnybowen-afk.github.io/Revly/annie/ (a beauty business site. Only offer it to beauty, hair, nails, lashes and similar businesses. For other industries, offer to send an example instead of linking this one.)
- **Footer credit on client sites:** Site by Sonny Bowen

## Pricing (don't change without updating the agents)
- **Build:** £595 one-off
- **Maintenance:** £50/month

## What the £50/month covers
The agents will only promise what's listed here.
- Hosting and keeping the site online
- Domain renewal
- Small updates when they ask (prices, hours, photos, menu changes)
- Security updates and backups

## Payment terms
- **Build fee due:** within 7 days of the site going live
- **Maintenance:** monthly, starting from the go-live date
- **Cancelling:** clients can cancel the £50 a month any time after the first month. So there's a one-month minimum and no long contract.
- **How clients pay:** bank transfer

## Domains
- Sonny buys and owns every client's domain, and the £50 a month covers renewing it.
- If a client asks who owns it, say Sonny does. What happens to the domain if a client cancels hasn't been decided yet, so don't promise anything and check with Sonny.

## Typical turnaround
- No clients yet, so nothing is fixed. Agents plan on a draft within 5 working days of getting the client's info, but **never promise a turnaround or a number of revision rounds** in a pitch. Check with Sonny if a client asks.

## How I contact leads
- **Email first** (saved as Gmail drafts), then **text (SMS)** to a mobile, then WhatsApp.
- **Avoid Instagram and Facebook DMs.** I don't want my Instagram banned. Only suggest a DM when there's no email or mobile, and flag it.

## Who I target
Businesses anywhere in England that have **no website** and **clear demand**, in three groups:
- **Tradesmen:** plumbers, electricians, builders, roofers, joiners and carpenters, plasterers, painters and decorators, landscapers and gardeners, tilers, kitchen and bathroom fitters, locksmiths, cleaners and similar.
- **Beauty:** hair salons, barbers, nail techs, lash and brow artists, beauty salons, aestheticians, spray tanning, mobile beauty and similar.
- **Housing developers:** small and local house builders and developers with current or upcoming developments.

**Clear demand** means visible signs the business is busy and customers want it:
- Recent posts, at least weekly, with regular job photos or new work.
- Customers asking questions or trying to book in comments and messages.
- A booking app with slots filling up.
- A Checkatrade, MyBuilder or Rated People profile with recent jobs.
- For developers: live or upcoming sites, or plots for sale.

A big review count can be a private signal of demand, but **never mention reviews in any message**. Skip businesses with no recent activity, even if they have no website.

## Hosting and launch
- **Hosting:** GitHub Pages (free) on Sonny's GitHub account `sonnybowen-afk`, with one repo per client called `site-<slug>`. The £50 a month covers keeping it online.
- **Domains:** Sonny buys every domain (`.co.uk` first) at his registrar and owns it.
- **Google Business Profile:** the client owns and verifies their own profile. Sonny just gets the website link added to it.

## Automation
- **Daily run:** every morning a scheduled run does the following, then writes today's briefing:
  - checks the pages inbox
  - researches new leads and drafts and checks their pitches
  - prepares follow-ups and chasers that are due
  - checks Gmail for replies
- **Leads per day:** 8 (two batches of 4).
- **What to research:** rotate the three target groups: tradesmen, then beauty, then housing developers. Start with Shrewsbury, then nearby Shropshire towns (Telford, Wellington, Oswestry, Ludlow, Bridgnorth, Market Drayton, Whitchurch, Newport, Church Stretton), then wider England. Never repeat a town and trade searched in the last 30 days (check `leads/`).
- **Auto-send cold emails: OFF.** While off, email pitches, follow-ups and chasers are saved as Gmail drafts for Sonny to send. Only Sonny can switch this on, by saying so.
- **Rules if auto-send is ON:**
  - Email only **limited companies and LLPs**, confirmed from the business's own details or Companies House. Never email sole traders or partnerships: UK PECR rules need their prior consent. Their pitches stay as Gmail drafts.
  - Every email says who Sonny is and ends with: "If you'd rather not hear from me, just reply 'no thanks' and I won't email again."
  - At most **10 new cold emails a day**, and at most **one follow-up** per business.
  - Never email anyone in `clients/do-not-contact.md`, anyone already in the pipeline for the same pitch, or any personal (non-business) address.
  - Anyone who replies asking not to be contacted goes straight into `clients/do-not-contact.md` and moves to Closed.
- **Always by hand:** texts, WhatsApps and Instagram, Facebook or LinkedIn DMs are always sent by Sonny himself. The run only prepares them.
